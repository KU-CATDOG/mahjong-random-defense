...
namespace MRD
{
    public class GukSaMuSangChecker : IYakuConditionChecker
    {
        ...
        public string[] OptionNames => new string[] { nameof(GukSaMuSangStatOption), nameof(GukSaMuSangOption) };

        public bool CheckCondition(YakuHolderInfo holder)
        {
            bool[] checker = Enumerable.Repeat<bool>(false,13).ToArray();
            int total = 0;
            foreach(Hai hai in holder.MentsuInfos.SelectMany(x => x.Hais)){
                int index = 0; total++;
            ...
            return total==14 && checker.All(x => x == true);
        }
    }
